...
class YogiPNM(Optimizer, BaseOptimizer):
    def __init__(
        self,
        params: PARAMETERS,
        lr: float = 1e-3,
    ...
    def step(self, closure: CLOSURE = None) -> LOSS:
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad
                if grad.is_sparse:
                    raise RuntimeError('YogiPNM does not support sparse gradients')
                ...
                ######
                previous_grad, previous_data = state['previous_grad'], state['previous_data']
