...
def CP(tensor: torch.Tensor, rank, modes=None, method='HALS', svd='truncated_svd', tol=10e-5):
    if tl.backend.BackendManager.get_backend() != 'pytorch':
        tl.backend.BackendManager.set_backend('pytorch')
    if method == 'HOSVD':
        return CP_HOSVD(tensor, rank=rank, modes=modes, svd=svd)
    elif method == 'HALS':
        return CP_HALS(tensor, rank=rank, modes=modes, svd=svd, tol=tol)
...
def CP_HOSVD(tensor, rank, modes=None, svd='truncated_svd'):
    if modes is None:
        modes = list(range(tl.ndim(tensor)))
    factors = []
    svd_fun = partial_svd
...
def CP_HALS(tensor, rank, modes=None, svd='truncated_svd', tol=10e-5):
    return non_negative_parafac_hals(tensor, rank=rank, modes=modes, svd=svd, tol=tol)
