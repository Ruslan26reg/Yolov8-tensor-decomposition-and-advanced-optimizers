...
def train(args, params):
    ...
    for v in model.modules():
        ...
        if isinstance(v, torch.nn.BatchNorm2d):
            ...
            p[0].append(v.weight)
    #optimizer = optim.YogiPNM(p[2], params['lr0'])
    #optimizer = optim.DiffPNM(p[2], params['lr0'])
    #optimizer = optim.PNMBelief(p[2], params['lr0'])
    #optimizer = optim.Yogi(p[2], params['lr0'])
    #optimizer = optim.DiffGrad(p[2], params['lr0'])
    ...
    filenames = []
    with open('../Dataset/filename/') as reader:
        for filename in reader.readlines():
            filename = filename.rstrip().split('/')[-1]
            filenames.append('../Dataset/filename/' + filename)
